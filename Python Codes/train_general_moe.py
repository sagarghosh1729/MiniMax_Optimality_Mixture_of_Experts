...
def compute_empirical_l2_measures(model, X_train, C=1.0, L=1.0):
    ...
    with torch.no_grad():
        _, expert_outputs, gate_outputs = model(X_train.to(device), return_components=True)
        ...
        bound = math.sqrt((denom/n 
                 + (d_in*d_out*math.log(C*L*gate_l2*n/denom))/n
...
def train_model(model, X_train, Y_train, X_test, Y_test, device, num_epochs = 50, lr = 1e-3, batch_size = 1024):
    device = get_device()
...
def main():
    ...
    for size in train_sizes:
        ...
        model = MixtureOfExperts(input_dim=X.shape[1], output_dim=Y.shape[1], hidden_dim=256, num_experts=4)
